...
xir::Op* add_conv(std::string name, std::shared_ptr<xir::Graph> graph,
  ...
  std::int32_t output_c = 3;
  auto input_shape = input->get_output_tensor()->get_shape();
  auto input_c = input_shape.back();
  auto attrs = xir::Attrs::create();
  attrs->set_attr<std::vector<int>>("kernel", {kernel_h, kernel_w});
  ...
  auto weights_data =
      std::vector<char>(output_c * kernel_h * kernel_w * input_c * 4, 1);
  weights_attrs
      ->set_attr<std::vector<std::int32_t>>(
          "shape", {output_c, kernel_h, kernel_w, input_c})
