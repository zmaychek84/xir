...
#if _WIN32
    char* expanded_attrs_library_list;
    size_t list_len;
    errno_t err = _dupenv_s(&expanded_attrs_library_list, &list_len,
                            "EXPANDED_ATTRS_LIBRARY");
    if (expanded_attrs_library_list != nullptr && err == 0 && list_len) {
#else
