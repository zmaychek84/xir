...
std::int64_t GraphImp::compute_model_size() const {
  v2::Serialize s{};
  return s.compute_model_size(this);
}

std::map<std::string, std::size_t> GraphImp::compute_graph_attribute_size()
    const {
  v2::Serialize s{};
  return s.compute_graph_attribute_size(this);
}
...
void GraphImp::serialize_to_memory(void* data, size_t size) const {
  v2::Serialize s{};
  s.write_to_memory(this, data, size);
}
