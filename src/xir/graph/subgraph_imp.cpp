...
std::set<Op*> SubgraphImp::get_input_ops() {
  return internal::cast_from_const_set(
      static_cast<const SubgraphImp&>(*this).get_input_ops());
}

const std::set<const Op*> SubgraphImp::get_input_ops() const {
...
std::set<Op*> SubgraphImp::get_output_ops() {
  return internal::cast_from_const_set(
      static_cast<const SubgraphImp&>(*this).get_output_ops());
}

const std::set<const Op*> SubgraphImp::get_output_ops() const {
  auto ret = std::set<const Op*>();
  for (auto op : this->ops_) {
...
Subgraph* SubgraphImp::find_op(const std::string& op_name) {
  ...
  if (this->has_op(op_name)) {
    ret = this;
    while (ret->get_children_num()) {
      for (auto& child : ret->get_children()) {
...
SubgraphImp::compute_subgraph_attribute_size() const {
  v2::Serialize s{};
  return s.compute_subgraph_attribute_size(this);
}
}  // namespace xir
