...
serial_v2::AttrValue convert_t<xir::any, serial_v2::AttrValue>::fun(
  ...
  if (it == dispatcher_from_any_to_pb.end()) {
    UNI_LOG_FATAL(XIR_UNSUPPORTED_TYPE) << "not supported type for attr."
                                        << "type_id: " << x.type().name();
  } else {
    ret = it->second(x);
  }
...
xir::any convert_t<serial_v2::AttrValue, xir::any>::fun(
  ...
  if (it == dispatcher_from_pb_to_any.end()) {
    UNI_LOG_FATAL(XIR_UNSUPPORTED_TYPE) << " AttrValue=" << x.DebugString();
  } else {
    ret = it->second(x);
...
unique_ptr<Graph> Serialize::read_from_memory(const void* data, size_t size) {
  serial_v2::Graph pb_graph;
  if (!pb_graph.ParseFromArray(data, (int)size)) {
    UNI_LOG_FATAL(XIR_READ_MEMORY_FAILURE)
        << "fail to generate graph from memory.";
  }
  return convert_from_pb_type_tp_cpp_type(
      *(static_cast<serial_v2::Graph*>(&pb_graph)));
}
...
std::int64_t Serialize::compute_model_size(const Graph* graph) {
  auto g = std::unique_ptr<Graph>(const_cast<Graph*>(graph));
  serial_v2::Graph pb_graph = convert_from_cpp_type_to_pb(g);
  g.release();
  return pb_graph.ByteSizeLong();
}

template <typename cpp_type>
std::map<std::string, std::size_t> get_attribute_size_map(const cpp_type* ori) {
  cpp_type* subgraph_non_const = const_cast<cpp_type*>(ori);
  auto pb_subgraph_attrs =
      convert_from_cpp_type_to_pb(subgraph_non_const->get_attrs());
  std::map<std::string, std::size_t> attribute_size_map;
  for (const auto& pair : pb_subgraph_attrs) {
    attribute_size_map.emplace(pair.first, pair.second.ByteSizeLong());
  }
  return attribute_size_map;
}

std::map<std::string, std::size_t> Serialize::compute_subgraph_attribute_size(
    const Subgraph* subgraph) {
  return get_attribute_size_map(subgraph);
}

std::map<std::string, std::size_t> Serialize::compute_op_attribute_size(
    const Op* op) {
  return get_attribute_size_map(op);
}

std::map<std::string, std::size_t> Serialize::compute_graph_attribute_size(
    const Graph* graph) {
  return get_attribute_size_map(graph);
}
...
void Serialize::write_to_memory(const Graph* graph, void* data, size_t size) {
  auto g = std::unique_ptr<Graph>(const_cast<Graph*>(graph));
  serial_v2::Graph memory_graph = convert_from_cpp_type_to_pb(g);
  g.release();
  if (!memory_graph.SerializeToArray(data, (int)size)) {
    UNI_LOG_FATAL(XIR_WRITE_MEMORY_FAILURE) << "fail to write data to memory.";
  }
}
