...
class OpImp : public Op {
 public:
  ...
  bool del_attr(const std::string& key) override;
  Op* rename(const std::string& name) override;
  ...
  // compute op attribute size
  std::map<std::string, std::size_t> compute_op_attribute_size() const override;
