...
void shape_infer_depthwise_conv1d(xir::Op* cur) {
  auto in = cur->get_input_tensor("input");
  auto in_shape = in->get_shape();
  auto out = cur->get_output_tensor();
  auto weights = cur->get_input_tensor("weights");
  ...
  UNI_LOG_CHECK(w_shape.size() == 3, XIR_INVALID_ARG_OCCUR)
      ...
      << ", but the number of the dimension should be 3.";
  auto attrs = cur->get_attrs();
  auto kernel = attrs->get_attr<std::vector<std::int32_t>>("kernel");
  auto stride = attrs->get_attr<std::vector<std::int32_t>>("stride");
  std::vector<std::int32_t> dilation = {1};
  ...
  auto ol = cal_out_1d(in_shape, padding, kernel, stride, dilation, pad_mode);
  auto out_channel = w_shape[0] * w_shape[2];
  std::vector<std::int32_t> new_out_shape = {in->get_shape().at(0), ol,
                                             out_channel};
  auto output_tensor =
      xir::Tensor::create(out->get_name(), new_out_shape, out->get_data_type());
...
void shape_infer_transposed_conv2d(xir::Op* cur) {
  ...
  if (pad_mode == "FLOOR") {
    ...
    // SAME
    // please ref to VAI-SRC-CONF-MAP-20
...
void shape_infer_mac(xir::Op* cur) {
  auto ins = cur->get_input_tensors();
  if (ins.size() == 3) {
    shape_infer_broadcast(cur);
  } else {
    UNI_LOG_FATAL(XIR_INVALID_ARG_OCCUR)
        << cur->to_string()  << "has input number of " << ins.size()
        << "but mac-fix only support 3term input.";
  }
}
...
void shape_infer_expand(xir::Op* cur) {
  auto output_tensor = cur->get_output_tensor();
  auto input_shape = cur->get_input_tensor("input")->get_shape();
  ...
  auto shape_op = shape_ops[0];
  UNI_LOG_CHECK(shape_op != nullptr, XIR_UNEXPECTED_VALUE)
        << cur->to_string() << "'s shape op is invalid.";
  UNI_LOG_CHECK(shape_op->has_attr("data"), XIR_UNEXPECTED_VALUE);
  auto shape = read_data_in_attr<std::int32_t>(shape_op);

  bool shape_le_input_shape = shape.size() >= input_shape.size();
  unsigned int output_shape_size = shape_le_input_shape? shape.size() : input_shape.size();
  unsigned int differ = shape_le_input_shape? shape.size()-input_shape.size():
                                              input_shape.size()-shape.size();
  std::vector<std::int32_t> output_shape;
  output_shape.resize(output_shape_size);
  for (unsigned int i = 0; i < output_shape_size; i++) {
    if (i < differ) {
      if (shape_le_input_shape) {
      ...
      if (shape_le_input_shape) {
        output_shape[i] = shape[i] > 1? shape[i] : input_shape[i-differ];
      } else {
        output_shape[i] = input_shape[i] > 1? input_shape[i] : shape[i-differ];
      }
    }
  }
  auto new_output_tensor = xir::Tensor::create(output_tensor->get_name(), output_shape,
...
void shape_infer_reciprocal(xir::Op* cur) { shape_infer_remain(cur); }
...
void shape_infer_fix(xir::Op* cur) { 
  shape_infer_remain(cur);
  auto bfp_method = cur->get_attr<std::string>("bfp_method");
  if (bfp_method != "none") {
    std::set<std::string> avail_method{"to_bfp", "to_bfp_v2",
                                       "to_bfp_prime_shared"};
    UNI_LOG_CHECK(avail_method.count(bfp_method) > 0, XIR_INVALID_ARG_OCCUR)
        << "invalid bfp method, can be: " << xir::to_string(avail_method)
        << " now is: " << bfp_method;
    auto block_size = cur->get_attr<std::int64_t>("block_size");
    UNI_LOG_CHECK(block_size > 0, XIR_INVALID_ARG_OCCUR)
        << "invlid block size, should be positive, now is: " << block_size;
    if (bfp_method == "to_bfp_prime_shared") {
      auto sub_block_size = cur->get_attr<std::int64_t>("sub_block_size");
      UNI_LOG_CHECK(sub_block_size > 0, XIR_INVALID_ARG_OCCUR)
        << "invlid sub_block_size, should be positive, now is: " << sub_block_size;
      auto sub_block_shift_bits = cur->get_attr<std::int64_t>("sub_block_shift_bits");
      UNI_LOG_CHECK(sub_block_shift_bits >= 0, XIR_INVALID_ARG_OCCUR)
          << "invlid sub_block_shift_bits, should be 0 or positive, now is: "
          << sub_block_shift_bits;
    }
    auto out_shape = cur->get_output_tensor()->get_shape();
    auto axis = cur->get_attr<int64_t>("axis");
    UNI_LOG_CHECK(
        axis >= -(int64_t)out_shape.size() && axis < (int64_t)out_shape.size(),
        XIR_INVALID_ARG_OCCUR)
        << "invalid bfp axis, should between "
        << -(int64_t)out_shape.size() << " and " << (int64_t)out_shape.size();
    auto convert_to_bfloat_before_bfp =
            cur->get_attr<std::int64_t>("convert_to_bfloat_before_bfp");
    UNI_LOG_CHECK(convert_to_bfloat_before_bfp >= 0, XIR_INVALID_ARG_OCCUR)
        << "invlid convert_to_bfloat_before_bfp, should be 0 or positive, "
        << "now is: " << convert_to_bfloat_before_bfp;
  }
}
...
void shape_infer_float2fix(xir::Op* cur) {
  ...
  if (cur->has_attr("bfp_method") &&
      cur->get_attr<std::string>("bfp_method") != "none") {
  ...
  new_output_tensor->set_attrs(cur->get_attrs());
  cur->replace_output_tensor(std::move(new_output_tensor));
}
...
void shape_infer_quantize_linear(xir::Op* cur) {
  DataType data_type{cur->get_attr<std::string>("data_type")};
  auto input_tensor = cur->get_input_tensor("input");
  auto output_tensor = cur->get_output_tensor();
  auto new_output_tensor = xir::Tensor::create(
      output_tensor->get_name(), input_tensor->get_shape(), data_type);
...
void shape_infer_dequantize_linear(xir::Op* cur) {
  auto input_tensor = cur->get_input_tensor("input");
  auto output_tensor = cur->get_output_tensor();
  auto new_output_tensor =
      xir::Tensor::create(output_tensor->get_name(), input_tensor->get_shape(),
...
void shape_infer_reduction(xir::Op* cur) {
  ...
  std::vector<std::int32_t> out_shape;
  auto is_bitmap_all_vaild = std::all_of(bitmap.begin(), bitmap.end(),
                                         [&](bool i) { return true == i; });
  if (is_bitmap_all_vaild && !keep_dims) {
...
void shape_infer_reduction_mean_fix(xir::Op* cur) {
...
void shape_infer_reduction_sum_fix(xir::Op* cur) {
  shape_infer_reduction_sum(cur);
}
...
void forward_reduction_min(xir::Op* cur) {
  auto inv = cur->get_input_tensor("input", 0);
  std::vector<int> shape{};
  if (inv->has_attr("shape_info"))
    shape = inv->get_attr<std::vector<std::int32_t>>("shape_info");
  ...
  for (auto s : shape)
    if (s < min) min = s;
  auto out = cur->get_output_tensor();
  out->set_attr("shape_info", std::vector<int>({min}));
}

void shape_infer_reduction_min(xir::Op* cur) {
  shape_infer_reduction(cur);
  forward_reduction_min(cur);
}

void shape_infer_reduction_min_fix(xir::Op* cur) {
  shape_infer_reduction_min(cur);
}

void shape_infer_qlinear_reduction_sum(xir::Op* cur) { shape_infer_reduction_sum_fix(cur); }
void shape_infer_qlinear_reduction_mean(xir::Op* cur) { shape_infer_reduction_mean_fix(cur); }
void shape_infer_qlinear_reduction_max(xir::Op* cur) { shape_infer_reduction_max_fix(cur); }
void shape_infer_qlinear_reduction_min(xir::Op* cur) { shape_infer_reduction_min_fix(cur); }
void shape_infer_qlinear_matmul(xir::Op* cur) { shape_infer_matmul_fix(cur); }
...
void shape_infer_space_to_depth(xir::Op* cur) {
  auto block_size = cur->get_attr<std::int32_t>("block_size");
  auto data_format = cur->get_attr<std::string>("data_format");
  auto in_shape = cur->get_input_tensor("input")->get_shape();
  UNI_LOG_CHECK(data_format == "NHWC", XIR_INVALID_ARG_OCCUR)
      << cur->to_string() << "'s data_format is " << data_format
      << ", but xir only support data_format is NHWC.";
  auto ic = in_shape[3];
  auto iw = in_shape[2];
  auto ih = in_shape[1];
  auto block_size_sq = block_size * block_size;
  std::int32_t oc = ic * block_size_sq;
  std::int32_t ow = iw / block_size;
  std::int32_t oh = ih / block_size;
  UNI_LOG_CHECK(iw % block_size == 0, XIR_INVALID_ARG_OCCUR)
      << "The number of input width for tiling layer must be multiples "
      << "of the block_size.";
  UNI_LOG_CHECK(ih % block_size == 0, XIR_INVALID_ARG_OCCUR)
      << "The number of input height for tiling layer must be multiples "
      << "of the block_size.";
  std::vector<std::int32_t> out_shape = {in_shape[0], oh, ow, oc};
  auto out = cur->get_output_tensor();
...
void shape_infer_layernorm(xir::Op* cur) { shape_infer_remain(cur); }
...
void shape_infer_equal(xir::Op* cur) { shape_infer_broadcast(cur); }

void shape_infer_greater(xir::Op* cur) { shape_infer_broadcast(cur); }

void shape_infer_greater_equal(xir::Op* cur) { shape_infer_broadcast(cur); }

void shape_infer_less(xir::Op* cur) { shape_infer_broadcast(cur); }

void shape_infer_less_equal(xir::Op* cur) { shape_infer_broadcast(cur); }

void shape_infer_or(xir::Op* cur) { shape_infer_broadcast(cur); }

void shape_infer_and(xir::Op* cur) { shape_infer_broadcast(cur); }
...
void shape_infer_argmax(xir::Op* cur) {
  ...
  auto out_shape = in->get_shape();
  out_shape[axis] = 1;
  if (attrs->has_attr("keepdims")) {
    auto keepdims = attrs->get_attr<std::int32_t>("keepdims");
    if(0 == keepdims) {
...
void shape_infer_argmin(xir::Op* cur) {
  auto in = cur->get_input_tensor("input");
  auto axis = cur->get_attr<std::int32_t>("axis");
  axis = axis < 0 ? axis + in->get_shape().size() : axis;
  auto out_shape = in->get_shape();
...
void forward_strided_slice(xir::Op* cur, std::vector<std::int32_t> begin,
  ...
  for (int32_t i = 0; i < in_size; i++) {
    ...
    for (int j = 0; j < in_shape_num; j++) {
      if ((stride[j] > 0 && (x / idx_stride[j] < begin[j] || x / idx_stride[j] >= end[j])) || (j == i && stride[j] < 0 && (x / idx_stride[j] > begin[j])) || 
          abs(x - begin[j]) % idx_stride[j] != 0) {
        if_push = false;
        break;
      }
      x = x % idx_stride[j];
...
void shape_infer_matmul_fix(xir::Op* cur) {
  auto inputs = cur->get_input_ops("input");
  auto weights = cur->get_input_ops("weights");
  UNI_LOG_CHECK((inputs.size() == 1 && weights.size() == 1) || 
                (inputs.size() == 2 && weights.size() == 0), XIR_INVALID_ARG_OCCUR)
      << "\"" << cur->get_name() << "\"(\"" << cur->get_type()
      << "\") requires more than two inputs, but there's " << inputs.size()
      << " input(s) in the graph.";
  auto bias = cur->get_input_ops("bias");
  UNI_LOG_CHECK(bias.size() <= 1, XIR_INVALID_ARG_OCCUR)
      << cur->to_string() << " has more than one bias.";
  auto op_mat_a = inputs[0];
  auto op_mat_b = weights.empty() ? inputs[1] : weights[0];
  auto mat_a_tensor = op_mat_a->get_output_tensor();
  auto mat_b_tensor = op_mat_b->get_output_tensor();
  auto mat_a_size = mat_a_tensor->get_shape();
  auto mat_b_size = mat_b_tensor->get_shape();
...
void shape_infer_qlinear_conv2d(xir::Op* cur) { shape_infer_conv2d_fix(cur); }

void shape_infer_qlinear_transposed_conv2d(xir::Op* cur) { shape_infer_transposed_conv2d_fix(cur); }

void shape_infer_qlinear_depthwise_conv2d(xir::Op* cur) {
  shape_infer_depthwise_conv2d_fix(cur);
}
...
void shape_infer_mac_fix(xir::Op* cur) { shape_infer_mac(cur); }

void shape_infer_qlinear_eltwise(xir::Op* cur) { shape_infer_eltwise_fix(cur); }
...
void shape_infer_qlinear_pool(xir::Op* cur) { shape_infer_pool_fix(cur); }
...
void shape_infer_clamp(xir::Op* cur) { shape_infer_remain(cur); }

void shape_infer_round(xir::Op* cur) { shape_infer_remain(cur); }

void shape_infer_sqrt(xir::Op* cur) { shape_infer_remain(cur); }

void shape_infer_not(xir::Op* cur) { shape_infer_remain(cur); }

void shape_infer_abs(xir::Op* cur) { shape_infer_remain(cur); }

void shape_infer_channel_shuffle(xir::Op* cur) {
  auto in = cur->get_input_tensor("input");
  auto out = cur->get_output_tensor();
  auto its = cur->get_input_tensors();
  auto group = cur->get_attr<std::int32_t>("group");
  UNI_LOG_CHECK(group > 0, XIR_UNEXPECTED_VALUE)
      << "The value of group here is " << group
      << ", but the value of group should be greater than 0.";
  auto out_shape = in->get_shape();
  auto output_tensor =
      xir::Tensor::create(out->get_name(), out_shape, out->get_data_type());
  output_tensor->set_attrs(out->get_attrs());
...
void shape_infer_pow(xir::Op* cur) { shape_infer_broadcast(cur); }

void shape_infer_where(xir::Op* cur) { shape_infer_broadcast(cur); }

static void shape_infer_gather_impl(xir::Op * cur, const std::vector<std::int32_t> &idx_shape) {
  auto in = cur->get_input_tensor("input");
  auto in_shape = in->get_shape();
  int in_rank = static_cast<int>(in_shape.size());
  int axis = 0;
  if (cur->has_attr("axis")) {
    axis = cur->get_attr<int>("axis");
  }
  bool scalar_index = false;
  if (cur->has_attr("scalar_index")) {
    scalar_index = cur->get_attr<bool>("scalar_index");
  }
  if (scalar_index) {
    UNI_LOG_CHECK((idx_shape.size() == 1) && (idx_shape[0] == 1),
                  XIR_UNEXPECTED_VALUE)
        << "gather op can only treat tensor shape of [1] as scalar.";
  }
  UNI_LOG_CHECK((axis >= -in_rank) && (axis <= in_rank - 1),
                XIR_UNEXPECTED_VALUE)
      << "gather op axis out of bound.";
  ...
  if (!scalar_index) {
    for (const auto dim : idx_shape) {
      out_shape.push_back(dim);
    }
  }
  for (auto i = axis + 1; i < in_rank; ++i) {
    out_shape.push_back(in_shape[i]);
  }
  if (out_shape.empty()) {
    // scalar tensor output, since XIR don't have that, use [1]
    out_shape.push_back(1);
  }
  auto out = cur->get_output_tensor();
  auto output_tensor =
...
void shape_infer_gather(xir::Op* cur) {
  /// indices as inputs
  auto idx = cur->get_input_tensor("indices");
  auto idx_shape = idx->get_shape();
  shape_infer_gather_impl(cur, idx_shape);
}

void shape_infer_quantize_gather(xir::Op* cur) {
  /// indices_shape as attributes
  auto idx_shape = cur->get_attr<vector<int>>("indices_shape");
  shape_infer_gather_impl(cur, idx_shape);
}
...
void shape_infer_broadcast_tile(xir::Op* cur) {
  auto input_shape = cur->get_input_tensor("input")->get_shape();
  auto repeats_ops = cur->get_input_ops("repeats");
  UNI_LOG_CHECK(repeats_ops.size() == 1, XIR_INVALID_ARG_OCCUR)
      << cur->to_string() << " requires one and only one repeats arg.";
  auto repeats_op = repeats_ops[0];
  UNI_LOG_CHECK(repeats_op != nullptr, XIR_UNEXPECTED_VALUE)
      << cur->to_string() << "'s repeats op is invalid.";
  // Only for support const repeats, will get rid of this check if repeats
  // from non-const op
  UNI_LOG_CHECK(repeats_op->has_attr("data"), XIR_UNEXPECTED_VALUE)
      << cur->to_string() << "'s repeats op must have data attr.";
  auto repeats = read_data_in_attr<std::int32_t>(repeats_op);

  // Support broadcast
  unsigned int output_shape_size = input_shape.size() >= repeats.size()?
                                   input_shape.size() : repeats.size();
  unsigned int differ = std::abs(static_cast<int>(input_shape.size()) -
                                 static_cast<int>(repeats.size()));
 
  std::vector<std::int32_t> new_shape;
  if (input_shape.size() > repeats.size()) {
    std::vector<std::int32_t> copy_repeats(repeats);
    new_shape.assign(copy_repeats.begin(), copy_repeats.end());
  } else if (input_shape.size() < repeats.size()) {
    std::vector<std::int32_t> copy_input_shape(input_shape);
    new_shape.assign(copy_input_shape.begin(), copy_input_shape.end());
  }
  if (differ) {
    new_shape.insert(new_shape.begin(), differ, 1);
  }

  std::vector<std::int32_t> output_shape;
  for(unsigned int i = 0; i < output_shape_size; ++i) {
    if (input_shape.size() > repeats.size()) {
      output_shape.push_back(input_shape[i] * new_shape[i]);
    } else if (input_shape.size() < repeats.size()) {
      output_shape.push_back(repeats[i] * new_shape[i]);
    } else {// input_shape.size() == repeats.size(), no broadcast.
      output_shape.push_back(repeats[i] * input_shape[i]);
    }
  }
  auto output_tensor = cur->get_output_tensor();
  auto new_output_tensor =
      xir::Tensor::create(output_tensor->get_name(), output_shape,
                          output_tensor->get_data_type());
  new_output_tensor->set_attrs(output_tensor->get_attrs());
  cur->replace_output_tensor(std::move(new_output_tensor));
}
