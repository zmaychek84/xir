...
Op* OpImp::rename(const std::string& name) {
  if (this->name_ != name) {
    UNI_LOG_CHECK(graph_->get_op(name) == nullptr, XIR_MULTI_REGISTERED_OP)
        << this->name_ << " can't be renamed to " << name
        << ", because this name is already used as."
        << graph_->get_op(name)->to_string();
  }
  this->name_ = name;
  return this;
}
...
bool OpImp::del_attr(const std::string& key) {
  UNI_LOG_CHECK(
      std::none_of(def_->attrs().begin(), def_->attrs().end(),
                   [key](const AttrDef& attr) { return attr.name == key; }),
      XIR_INVALID_ATTR_DEF)
      << "attr " << key << " is protected and can't be deleted.";
  return attrs_->del_attr(key);
}
...
std::map<std::string, std::size_t> OpImp::compute_op_attribute_size() const {
  v2::Serialize s{};
  return s.compute_op_attribute_size(this);
}
