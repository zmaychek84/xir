...
struct AttrHelper {
  ...
  static py::object get(const Object* obj, const std::string& key) {
    ...
    } else if (type == xir::TYPE_INDEX_MAP_STR_2_VEC_CHAR) {
          py::dict dict;
          auto data = obj->template get_attr<std::map<std::string, std::vector<char>>>(key);
          for (auto &item : data){
            dict[py::str(item.first)] = py::bytes(item.second.data(), item.second.size());
          }
          return dict;
