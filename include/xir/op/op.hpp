...
class XIR_DLLESPEC Op {
 public:
   ...
   * the attributes defined in OpDef are protected and can't be removed.
   *
   ...
   * @brief Rename the Op
   *
   * @param name The new name for the op.
   *
   * @return A raw pointer to the current Op object.
   *
   */
  virtual Op* rename(const std::string& name) = 0;
  ...
  /**
   * @brief Get the OpDef of this Op.
   *
 ...
 public:
  ...
  /**
   * @brief Compute the op attribute size.
   *
   * @return A map whose key is op attribute name and value is op attribute
   * size.
   */
  virtual std::map<std::string, std::size_t> compute_op_attribute_size()
      const = 0;
};
