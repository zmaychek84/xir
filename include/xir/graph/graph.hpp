...
class XIR_DLLESPEC Graph {
 public:
  ...
  /**
   * @brief Compute the model size.
   *
   * @return An int64 model size.
   */
  std::int64_t compute_model_size() const;
  ...
  /**
   * @brief Deserializa a graph from a memory.
   *
   * @param data Pointer in memory of binary content.
   * @param size Size of the memory of binary content.
   *
   * @return A unique pointer to the graph object.
   */
  static std::unique_ptr<Graph> deserialize_from_memory(const void* data,
                                                        size_t size);
  ...
  /**
   * @brief Compute the graph attribute size.
   *
   * @return A map whose key is graph attribute name and value is graph
   * attribute size.
   */
  std::map<std::string, std::size_t> compute_graph_attribute_size() const;
 ...
 public:
   ...
   * @brief Serialize the graph.
   *
   * @return A void* storing the graph.
   */
  virtual void serialize_to_memory(void* data, size_t size) const = 0;
};
