...
class XIR_DLLESPEC Subgraph {
 public:
  ...
  /**
   * @brief Get all the input ops of this subgraph.
   *
   * @return A set of raw pointer to the input ops.
   */
  virtual std::set<Op*> get_input_ops() = 0;
   ...
   * @return A set of raw pointer to the input ops.
   */
  virtual const std::set<const Op*> get_input_ops() const = 0;

  /**
   * @brief Get all the output ops of this subgraph.
   *
   * @return A set of raw pointer to the output ops.
   */
  virtual std::set<Op*> get_output_ops() = 0;
  /**
   * @brief Get all the output ops of this subgraph.
   *
   * @return A set of raw pointer to the output ops.
   */
  virtual const std::set<const Op*> get_output_ops() const = 0;
 ...
 public:
  ...
  /**
   * @brief Compute the subgraph attribute size.
   *
   * @return A map whose key is subgraph attribute name and value is subgraph
   * attribute size.
   */
  virtual std::map<std::string, std::size_t> compute_subgraph_attribute_size()
      const = 0;
};
